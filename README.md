...
A Python-based automation tool that extracts stock images and metadata from dynamically loaded, infinite-scroll websites.

🔹 Uses `Selenium` to handle JavaScript execution and bypass infinite scrolling
...
## 🎥 YouTube Scraping & Data Analysis

An end-to-end Python pipeline that automates the collection, cleaning, and visualization of YouTube video data.

🔹 Uses `Selenium` to scrape video metadata including titles, view counts, durations, and channels
🔹 Cleans and preprocesses shorthand text metrics (e.g., converting "1.5M" to numerical formats) using `Pandas`
🔹 Generates exploratory visual insights like word clouds and top-viewed distributions using `Matplotlib` and `Seaborn`

👉 [View Project](https://github.com/vinayakmishra4/Project-Youtube-Scrapping)
